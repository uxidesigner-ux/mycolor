...
### App Shell: Device Frame (v0.2.6)

레이아웃의 단일 진실원천은 **하나의 디바이스 프레임**이다. 모든 화면과 오버레이는 이 프레임 안에서만 동작한다.

- **데스크톱(≥861px):** RoutineDiary식 2단 구성. 왼쪽은 항상 고정된 서비스 소개(`.app-aside`), 오른쪽은 고정 폭(~404px) 폰 프레임(`.device-frame`). 프레임은 둥근 모서리·경계선·그림자를 가진 단일 디바이스다.
- **모바일(≤860px):** 소개는 숨고, 프레임이 풀블리드(테두리 0, 100svh)로 전환되어 기존 풀스크린 앱과 동일하게 보인다.
- **봉쇄 원칙:** `.device-stage`가 `overflow:hidden`으로 모든 화면·플로우 시트·바텀시트·토스트·홈 내비를 프레임에 클립한다. 어떤 오버레이도 뷰포트가 아니라 **프레임 기준**으로 위치(`position:absolute`)한다 — 바텀시트가 프레임보다 커지지 않는다.
- **내부 스크롤:** 각 화면은 프레임 내부에서 스크롤한다(윈도우 스크롤 아님). 프레임 상단 chrome(`.site-header`: 브랜드·저장)은 고정되고, 홈에서는 숨겨 자체 앱바를 쓴다.
- **컨테이너 = 폭의 기준:** 프레임은 항상 좁으므로(~404px), 데스크톱 다단 그리드는 프레임 안에서 절대 쓰지 않는다. 결과 요약·분석 그리드 등은 프레임 안에서 항상 좁은(모바일) 레이아웃으로 렌더한다.

### Header Actions (v0.2.7)

프레임 상단 헤더는 모든 화면에 지속되는 앱 chrome이다.

- **좌측:** 브랜드(홈 링크) + 로그인 버튼. 로그아웃 상태는 구글 G 아이콘 + `로그인`, 로그인 상태는 아바타(사진 없으면 이니셜) + 이름의 다크 pill, 클릭 시 계정 시트(로그아웃).
- **우측:** 저장(저장된 스타일이 있을 때만) · 새로고침 · 공유 아이콘 버튼.
- **로그인:** 구글 신원 확인만(Google Identity Services). 서버 DB·세션 없음 — 프로필은 localStorage에 표시용으로만 보관. OAuth Client ID는 `config.googleClientId`(공개값)로 주입.
- **공유:** 완성된 결과가 있으면 스타일 공유, 없으면 앱 공유.
- 홈은 이 슬림 유틸 헤더 아래에 자체 앱바(스타일 시작)를 그대로 둔다.

### Records & Route (v0.2.13)

앱의 본질은 **집 → 밖으로 나가는 준비**. 그래서 두 가지를 더한다.

- **기록(History) vs 저장(Saved):** 조회·생성한 추천은 자동으로 **기록**에 쌓이고(타임라인), 결과의 **"이 제안 저장하기"** 버튼으로 담은 것만 **저장**에 남는다(날짜별 큐레이션). 내비 좌측은 `홈 · 기록 · 저장`. 둘 다 바텀시트 리스트로 열고, 항목을 탭하면 그 프로필로 결과를 다시 연다. 저장 항목은 삭제 가능. 저장소는 `moi-history-v1`(cap 50)·`moi-saved-v1`.
- **출발지 → 도착지 날씨:** 날씨는 "지금 여기"가 아니라 **경로**다. 출발지(기본=현재 위치) + 도착지(선택). 두 지점 날씨를 모두 조회해 **합(기온 범위·강수/자외선 최댓값)**으로 추천 노트를 만든다. 홈 상단에 출발지 오늘 날씨 카드(⚙ → 출발지/도착지 설정 시트), 결과 "오늘" 탭 칩도 `출발지 → 도착지`로 표시. 도착지는 기존 "활동 지역"과 동일 값. 경로는 `moi-route-v1`.

### Weather Layer (v0.2.12)

프로필은 두 레이어다: **고정 정체성**(얼굴형·퍼스널컬러·추천기준)과 **오늘의 컨텍스트**(무드·날씨). 날씨는 매일 변하는 변수이므로 전체를 바꾸지 않고 **"오늘" 탭만 모듈레이트**한다.

- **표면:** "오늘" 탭 최상단에 날씨 칩(지역·현재 기온·상태·최고/최저·습도·자외선) + 그 아래 2–4개의 조정 노트(헤어/뷰티/자외선/옷). 정체성 추천 위에 한 겹 얹는 형태.
- **데이터:** Open-Meteo(무료·키 불필요·CORS). 위치는 지오로케이션 우선, 없으면 "활동 지역" 텍스트를 지오코딩. 좌표/지역 + 1시간 TTL로 localStorage 캐시. 백엔드·DB 없음.
- **규칙 기반:** 기온·습도·강수확률·자외선·바람 임계값 → 노트 매핑(AI 미사용). 날씨 없으면(권한 거부·오프라인) 기존 추천 그대로 유지(graceful degradation).
- 다른 탭(헤어/뷰티/옷/관리 상세)은 날씨로 바꾸지 않는다 — 날씨는 "오늘" 중심.
...
- 데스크톱에서는 왼쪽 고정 서비스 소개와 오른쪽 폰 프레임을 나란히 두고, 앱 홈은 프레임 안에서 동작한다(App Shell 참고). 실제 사용 화면과 웹 진입 화면의 멘탈모델을 맞춘다.
